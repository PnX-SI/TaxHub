...
sudo -u postgres -s psql -h $db_host -U postgres -d $db_name  -f scripts/0.1.2_import_taxrefv13.sql   &>> $LOG_DIR/update_taxref_v13.log

echo "Export des cd_nom à modifier dans les données d'observations"
sudo -u postgres -s psql -h $db_host -U postgres -d $db_name  -f scripts/0.1.3_cd_nom_disparu_synthese_export.sql &>> $LOG_DIR/update_taxref_v13.log
...
sudo -u postgres -s psql -h $db_host -U postgres -d $db_name  -f scripts/1.3_taxref_changes_detections_export.sql &>> $LOG_DIR/update_taxref_v13.log
