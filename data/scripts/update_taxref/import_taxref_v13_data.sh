...
array=( TAXREF_INPN_v13.zip ESPECES_REGLEMENTEES_v13.zip BDC_STATUTS_13.zip)
for i in "${array[@]}"
do
    if [ ! -f '/tmp/taxhub/'$i ]
    then
...
export PGPASSWORD=$user_pg_pass;psql -h $db_host -U $user_pg -d $db_name  -f scripts/1.3_taxref_changes_detections_export.sql &>> $LOG_DIR/update_taxref_v13.log
