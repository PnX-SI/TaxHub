...
CREATE OR REPLACE FUNCTION match_binomial_taxref(my_taxa_name character varying)
RETURNS integer
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
DECLARE 
	matching_cd integer;
BEGIN
	IF (SELECT count(DISTINCT cd_nom) FROM taxonomie.taxref WHERE lb_nom=my_taxa_name)=1 THEN matching_cd:= cd_nom FROM taxonomie.taxref WHERE lb_nom=my_taxa_name OR nom_valide=my_taxa_name ;
	ELSIF (SELECT count(DISTINCT cd_ref) FROM taxonomie.taxref WHERE lb_nom=my_taxa_name)=1 THEN matching_cd:= DISTINCT(cd_ref) FROM taxonomie.taxref WHERE lb_nom=my_taxa_name OR nom_valide=my_taxa_name ;
