...
app
  ...
  .run([
    ...
    function (
      ...
      // Set HTTP Authorization header with JWT
      $http.defaults.headers.common.Authorization =
        "Bearer " + loginSrv.getToken();
      $rootScope.$on("$routeChangeStart", function (event, next, current) {
        ...
        if (next.access.restricted) {
          next.access.level === undefined
            ? (level = 0)
            : (level = next.access.level);
          if (
            loginSrv.getToken() !== undefined &&
            level <= loginSrv.getCurrentUser().id_droit_max
          )
            return;
          toaster.pop(
            "error",
