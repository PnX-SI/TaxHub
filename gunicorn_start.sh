...
then
  exec gunicorn  server:app --certfile="${https_cert_path}" --keyfile="${https_key_path}" 
    --access-logfile /var/log/taxhub/taxhub-access.log --error-log /var/log/taxhub/taxhub-errors.log
    --pid="${app_name}.pid" -w "${gun_num_workers}"  -b "${gun_host}:${gun_port}"  -n "${app_name}"
    --bind 0.0.0:443
else
