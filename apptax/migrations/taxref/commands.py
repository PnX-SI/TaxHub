...
def update_taxref_v15():
    ...
    query = text(importlib.resources.read_text('apptax.migrations.data.migrate_taxref_version', '0.2_taxref_detection_repercussion_disparition_cd_nom.sql'))
...
def apply_changes(keep_taxref, keep_bdc):
    ...
    try:
        query = text(importlib.resources.read_text('apptax.migrations.data.migrate_taxref_version', '3.2_alter_taxref_data.sql'))
        db.session.execute(query)
        db.session.commit()
    ...
    query = text(importlib.resources.read_text('apptax.migrations.data.migrate_taxref_version', '5_clean_db.sql'))
    db.session.execute(query)
...
def import_taxref_v15():
    ...
    for sqlfile in ['0_taxrefv15_import_data.sql',]:
        query = text(importlib.resources.read_text('apptax.migrations.data.migrate_taxref_version', sqlfile))
        db.session.execute(query)
    db.session.commit()
