        ...
        """
            ...
            FROM ( SELECT t_1.cd_nom,
                    ...
                    SELECT DISTINCT t_1.cd_nom,
                        t_1.cd_ref,
                        concat(split_part(t_1.nom_vern::text, ','::text, 1), COALESCE(t_1.nom_vern, '=', ''), '<i>', t_1.nom_valide, '</i>', ' - [', t_1.id_rang, ' - ', t_1.cd_ref, ']') AS search_name,
                        t_1.nom_valide,
                        t_1.lb_nom,
                        t_1.nom_vern,
                        t_1.regne,
...
def downgrade():
    op.execute(
        ...
        FROM ( SELECT t_1.cd_nom,
                ...
                SELECT DISTINCT t_1.cd_nom,
                    t_1.cd_ref,
                    concat(split_part(t_1.nom_vern::text, ','::text, 1), COALESCE(t_1.nom_vern, '='::character varying, ''::character varying), '<i>', t_1.nom_valide, '</i>', ' - [', t_1.id_rang, ' - ', t_1.cd_ref, ']') AS search_name,
                    t_1.nom_valide,
                    t_1.lb_nom,
                    t_1.nom_vern,
                    t_1.regne,
