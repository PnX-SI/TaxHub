...
CREATE MATERIALIZED VIEW IF NOT EXISTS taxonomie.vm_regne AS (SELECT DISTINCT regne FROM taxonomie.taxref tx) WITH DATA;
CREATE MATERIALIZED VIEW IF NOT EXISTS taxonomie.vm_phylum AS (SELECT DISTINCT phylum FROM taxonomie.taxref tx) WITH DATA;
CREATE MATERIALIZED VIEW IF NOT EXISTS taxonomie.vm_classe AS (SELECT DISTINCT classe FROM taxonomie.taxref tx) WITH DATA;
CREATE MATERIALIZED VIEW IF NOT EXISTS taxonomie.vm_ordre AS (SELECT DISTINCT ordre FROM taxonomie.taxref tx) WITH DATA;
CREATE MATERIALIZED VIEW IF NOT EXISTS taxonomie.vm_famille AS (SELECT DISTINCT famille FROM taxonomie.taxref tx) WITH DATA;
CREATE MATERIALIZED VIEW IF NOT EXISTS taxonomie.vm_group1_inpn AS (SELECT DISTINCT group1_inpn FROM taxonomie.taxref tx) WITH DATA;
CREATE MATERIALIZED VIEW IF NOT EXISTS taxonomie.vm_group2_inpn AS (SELECT DISTINCT group2_inpn FROM taxonomie.taxref tx) WITH DATA;
